...
int main( int argc, const char ** argv )
{
	...
	}
	{
		...
		doc->SaveFile( "../out/pretty.xml" );
		XMLTest( "Save pretty.xml", false, doc->Error() );
		doc->SaveFile( "../out/compact.xml", true );
	...
	{
		XMLDocument doc;
		XMLError error = doc.LoadFile( "empty.xml" );
...

	{
        ...
        static const char* xml_bom_preservation  = "\xef\xbb\xbf<element/>\n";
        {
			...
			doc.SaveFile( "../out/bomtest.xml" );
			XMLTest( "Save bomtest.xml", false, doc.Error() );
        }
		{
			XMLDocument doc;
			doc.LoadFile( "../out/bomtest.xml" );
    ...
    {
        const char* TESTS[] = {
            "./xmltest-5662204197076992.xml",     // Security-level performance issue.
            0
        };
        for (int i = 0; TESTS[i]; ++i) {
            XMLDocument doc;
